...
{
    coefA.resize(na + 1);
    coefA = Eigen::VectorXd::Zero(na + 1, 1);
    coefB.resize(nb + 1);
    coefB = Eigen::VectorXd::Zero(nb + 1, 1);
...
SModel d2c(const ArxModel& dModel, double Ts, DiscretizationMethod method)
{
    ...
    case Zoh:
        {
            ...
            int order = std::max(dModel.Na() + 1, dModel.Nb() + 1);
            auto t = Polynomial(Eigen::VectorXd::Identity(1,1));    //constant 1
            auto den = Polynomial(Eigen::Vector2d(0, 0));
            for(auto i = 0;i < order - dModel.Na() + 1;++i)
            {
                t = t * zoh;
            }
            for(auto i = 0;i < dModel.Na() + 1;++i)
            {
            ...
            for (auto i = 0; i < order - dModel.Nb() + 1; ++i)
            {
                t = t * zoh;
            }
            for(auto i = 0;i < dModel.Nb() + 1;++i)
            {
    ...
    case Tustin:
        //z = (1+s*Ts/2)/(1-s*Ts/2)
        {
            ...
            dens.resize(dModel.Na() + 1, Polynomial(Eigen::Vector2d(0, 0)));
            nums.resize(dModel.Na() + 1, Polynomial(Eigen::Vector2d(0, 0)));
            for (auto i = 0; i < order - dModel.Na() + 1; ++i)
            {
                tmpDen = tmpDen * denP0;
            }
            ...
            for(auto i = dModel.Na() + 1 - 1;i >= 0;--i)
            {
            ...
            for (auto i = dModel.Nb() + 1 - 1; i >= 0; --i)
            {
